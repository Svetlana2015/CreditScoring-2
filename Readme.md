*Это первый проект, который был отклонен. В приложении необходимо дать ответ на каждый вопрос клиента (сколько лет, какая ЗП, сколько детей и т.д.). В последствии приложение даёт ответ о вероятности возврата кредита или нет. 3 репозитория связаны между собой: FastAPI_4, streamlit_2, streamlit.  Разница в фишках streamlit разный url-адрес приложения. Из этих 3 репозиториев я создам один открытый, чтобы показать код. Но вряд ли приложение будет работать, так связи будут нарушены.*
